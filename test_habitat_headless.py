...
# 【关键】强制指定 EGL 无头渲染使用的 GPU，0代表第一张卡
...
class HabitatDreamerWrapper(gym.Env):
    def __init__(self, config_path, res=(64, 64)):
        self._env = habitat.Env(config=habitat.get_config(config_path))
        self._res = res
    ...
    def _process_obs(self, obs, reward, is_first, is_last, is_terminal):
        # 提取 RGB 图像并 Resize 到 Dreamer 指定大小 (例如 64x64)
        image = obs['rgb']
    ...
    def step(self, action):
        ...
        # 注意: 某些基础 config 可能没有自带 reward metric，这里做个兜底
...
if __name__ == "__main__":
    ...
    # 使用刚才下载的测试场景配置
    config_path = "habitat-lab/habitat-baselines/habitat_baselines/config/pointnav/ppo_pointnav_example.yaml"
    
    # 替换为你实际的测试任务 config，这里使用基础的 PointNav 配置
    test_config = "habitat-lab/habitat-lab/habitat/config/benchmark/nav/pointnav/pointnav_habitat_test.yaml"
