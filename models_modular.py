...
import networks_modular as networks
import tools
...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        self._scales = dict(
            ...
            heatmap=getattr(config, "full_heatmap_scale", 1.0),
            inverse=getattr(config, "inverse_loss_scale", 0.0),
            affordance_s=getattr(config, "affordance_s_scale", 0.0),
    ...
    def _train(self, data_origin):
        ...
        with tools.RequiresGrad(self):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                # --- [Module] S-Aff: affordance reconstruction from S only ---
                # Fully disabled when affordance_s_scale == 0.
                s_stoch_dim = self.dynamics._stoch_s * (
                    self.dynamics._discrete if self.dynamics._discrete else 1
                )
                s_feat_dim = s_stoch_dim + self.dynamics._deter_s
                ...
                if self._scales.get("affordance_s", 0.0) > 0.0:
                    if "heatmap" not in data:
                        raise KeyError(
                            "S-Aff is enabled but replay has no heatmap. "
                            "Enable the auxiliary heatmap pipeline."
                        )
                    feat_s_only = torch.zeros_like(feat_all)
                    feat_s_only[..., :s_feat_dim] = feat_all[..., :s_feat_dim]
                    preds_s = self.heads["decoder"](feat_s_only)
                    if "heatmap" not in preds_s:
                        raise KeyError(
                            "S-Aff is enabled but decoder does not predict heatmap."
                        )
                    loss_affordance_s = -preds_s["heatmap"].log_prob(data["heatmap"])
                else:
                    loss_affordance_s = feat_all[..., 0] * 0.0
