...
def build(outcome_enabled):
    return SOnlyMultiStepRSSMConsistency(
        ...
        outcome_balance_mode="balanced",
        outcome_positive_mix=0.5,
        outcome_positive_threshold=1.0e-6,
        outcome_calibration_enabled=True,
        outcome_calibration_scale=0.5,
    )
...
loss_base, out_base, proto_base, _ = mod_base(dyn_base, post, actions, is_first, rewards=rewards)
rng_base = torch.get_rng_state().clone()

torch.set_rng_state(state0)
loss_s5a, loss_out, proto_out, metrics = mod_out(dyn_out, post, actions, is_first, rewards=rewards)
...
assert float(proto_base) == 0.0 and float(proto_out) == 0.0
...
required = [
    ...
    "s_outcome_mae_h4",
    "s_outcome_corr_h4",
    "s_outcome_rollout_positive_count_h4",
    "s_outcome_rollout_negative_count_h4",
    "s_outcome_calibration_loss",
    "s_outcome_calibration_positive_count_h4",
...
# 5) Balanced loss must give rare positives a non-vanishing contribution when present.
mod_bal = build(True)
pred = torch.tensor([0.0, 0.0, 0.0, 0.0], requires_grad=True)
target = torch.tensor([1.0, 0.0, 0.0, 0.0])
mask = torch.ones(4, dtype=torch.bool)
loss_bal, stats_bal = mod_bal._outcome_regression_loss(pred, target, mask)
expected_pos = torch.log(torch.tensor(2.0)).square()
expected = 0.5 * expected_pos  # negative mean is exactly zero
assert torch.allclose(loss_bal.detach(), expected, atol=1e-6), (loss_bal, expected)
assert int(stats_bal["positive_count"].item()) == 1
assert int(stats_bal["negative_count"].item()) == 3
loss_bal.backward()
assert pred.grad is not None and torch.isfinite(pred.grad).all()
print("PASS: balanced Outcome loss prevents rare positive target dilution")
