...
class ClipReward(ABC):
    ...
    def _load_mineclip(self, ckpt, config):
        ...
        self.model.eval()
        # This branch treats MineCLIP as a frozen language/reward model.
    ...
    def get_task_embedding(self, prompts: List[str], prompt_index: int = 0) -> th.Tensor:
        ...
        This is computed once when the environment wrapper is constructed, not
        once per environment step. The positive task prompt is index 0.
        """
        if not prompts:
