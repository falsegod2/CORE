...
class ClipReward(ABC):
    def __init__(self, ckpt="weights/mineclip_attn.pth", **kwargs) -> None:
        ...
        self.feature_dim = int(kwargs["image_feature_dim"])
    ...
    def _load_mineclip(self, ckpt, config):
        ...
        self.model.eval()
        # MineCLIP is a frozen domain-semantic encoder in this variant. Its
        # parameters are never optimized by Dreamer and all environment-side
    ...
    def get_task_embedding(self, prompts: List[str], prompt_index: int = 0) -> th.Tensor:
        """Return the frozen MineCLIP text embedding for the active task."""
        if not prompts:
            raise ValueError("At least one task prompt is required")
        if not 0 <= prompt_index < len(prompts):
            raise IndexError(
                f"prompt_index={prompt_index} outside [0, {len(prompts)})"
            )
        with th.no_grad():
            text_feats = self._get_text_feats(prompts)
        embedding = text_feats[prompt_index].detach().float().cpu()
        if embedding.shape != (self.feature_dim,):
            raise RuntimeError(
                f"Expected task embedding ({self.feature_dim},), got "
                f"{tuple(embedding.shape)}"
            )
        return embedding

    def get_logits_and_embedding(
        self,
        obs: Dict,  # 3 x 160 x 256
        prompts: List[str],
        state: Tuple[th.Tensor, th.Tensor] = None,  # history x 512
    ) -> Tuple[th.Tensor, Tuple[th.Tensor, th.Tensor], th.Tensor]:
        """Return reward logits and the frozen global MineCLIP video embedding.

        The embedding is the same 512-D temporal feature already computed for
        the MineCLIP reward head, so exposing it does not add a second visual
        forward pass. It is detached and moved to CPU before entering the
        environment observation/replay path.
        """
        ...
        next_state = (image_feats[0, 1:].cpu(), text_feats.cpu())
        flat_video_feats = video_feats.reshape(-1, video_feats.shape[-1])
        if flat_video_feats.shape[0] != 1:
            raise RuntimeError(
                f"Expected one MineCLIP video feature, got {tuple(video_feats.shape)}"
            )
        global_embedding = flat_video_feats[0].detach().float().cpu()
        if global_embedding.shape != (self.feature_dim,):
            raise RuntimeError(
                f"Expected MineCLIP embedding ({self.feature_dim},), got "
                f"{tuple(global_embedding.shape)}"
            )
        return logits, next_state, global_embedding

    def get_logits(
        self,
        obs: Dict,
        prompts: List[str],
        state: Tuple[th.Tensor, th.Tensor] = None,
    ) -> Tuple[th.Tensor, Tuple[th.Tensor, th.Tensor]]:
        # Preserve the original reward-only API for other callers.
        logits, next_state, _ = self.get_logits_and_embedding(obs, prompts, state)
