...
def probability_to_bool(input_tensor, jump_prob=1.0, generator=None):
...
class ImagBehavior(nn.Module):
    def __init__(self, config, world_model):
        ...
        self._use_amp = True if config.precision == 16 else False
        self._config = config
        self._jump_generator = torch.Generator(device=config.device)
        self._jump_generator.manual_seed(int(config.seed) + 50001)
        ...
        with tools.RequiresGrad(self.actor):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                for _ in range (self._config.imag_horizon - 1):
                    ...
                    indices = probability_to_bool(jump_tensor * (1.0 - end_factor), self.jump_prob, self._jump_generator).squeeze() # [N]
                ...
                zoom_indices = probability_to_bool(zoom_indices, self.jump_prob, self._jump_generator).squeeze() # [L, N]
