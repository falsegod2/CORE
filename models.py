...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        # Experiment 5A: open-loop multi-step consistency on the existing RSSM.
        # No new latent branch, reward, object token, or actor/critic input is added.
        ms_cfg = getattr(config, "multi_step_consistency", {}) or {}
        self._multistep_enabled = bool(ms_cfg.get("enabled", False))
        self._multistep_scale = float(ms_cfg.get("loss_scale", 0.0))
        self._multistep = None
        if self._multistep_enabled:
            self._multistep = multistep_consistency.MultiStepRSSMConsistency(
                feat_dim=feat_size,
                horizons=ms_cfg.get("horizons", [1, 2, 4, 8, 15]),
                horizon_weights=ms_cfg.get(
    ...
    def _train(self, data_origin):
        ...
        with tools.RequiresGrad(self):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                multistep_metrics = {}
                multistep_loss = model_loss.mean() * 0.0
                if self._multistep_enabled:
                    multistep_loss, multistep_metrics = self._multistep(
    ...
    '''原版做了极其复杂的动作维度补齐（补 0 和补 1）以及热力图解析
...
class ImagBehavior(nn.Module):
    def __init__(self, config, world_model):
        ...
        self._use_amp = True if config.precision == 16 else False
        self._config = config
        '''删除 self.jump_prob 和 self.gamma_sum 的定义。
    ...
    def _imagine(self, start, policy, horizon):
        ...
        def step(prev, _):
            ...
            '''删除拼接 zero_tensor 的那两行代码，直接传 action
